:- dynamic(suggested/1).
:- dynamic(done/1).
:- dynamic(branch_inp/1).
:- dynamic(done_inp/1).
:- dynamic(elective_inp/1).
:- dynamic(interest_inp/1).
:- dynamic(eco_minor_inp/1).
:- dynamic(ent_minor_inp/1).
...
get_rec() :-
    ...
    ,nl,branch_inp(Branch),write(Branch),nl,
    get_core(Branch).
...
interactive_get_core(TBD,DONE,[X|Y]):-
    done_inp(X),write("done"),nl,check_res(y,TBD,DONE,[X|Y]).
interactive_get_core(TBD,DONE,[X|Y]):-
    \+done_inp(X),check_res(n,TBD,DONE,[X|Y]).

eco_minor():-
    nl,write("do you want a minor degree in Economics?"),
    nl,eco_minor_inp(ECO),
    \+check_eco(ECO),
    more_eco_recs(ECO).
...
ent_minor():-
    ...
    nl,ent_minor_inp(ENT),\+check_ent(ENT),more_ent_recs(ENT).
